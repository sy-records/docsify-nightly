(function () {
  ...
  const isAbsolutePath = cached$2(path => {
    return /(:|(\/{2}))/g.test(path);
  });

  const removeParams = cached$2(path => {
    return path.split(/[?#]/)[0];
  });

  const getParentPath = cached$2(path => {
    if (/\/$/g.test(path)) {
      return path;
    }
  ...
  const cleanPath = cached$2(path => {
    return path.replace(/^\/+/, '/').replace(/([^:])\/{2,}/g, '$1/');
  });

  const resolvePath = cached$2(path => {
  ...
  const replaceSlug = cached$2(path => {
  ...
  function getAlias(path, alias, last) {
    const match = Object.keys(alias).filter(key => {
      const re = cached$1[key] || (cached$1[key] = new RegExp(`^${key}$`));
  ...
  function scrollActiveSidebar(router) {
    ...
    off('scroll', () => highlight(path));
    on('scroll', () => highlight(path));
  ...
  const scrollEl = $.scrollingElement || $.documentElement;

  function scroll2Top(offset = 0) {
